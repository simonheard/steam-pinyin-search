...
4. Wikidata is the community alias layer: structured data is CC0, Steam AppID is property P1733, and Chinese labels/aliases are joined without guessing identities. Steam/PICS titles have precedence, then Wikidata may fill missing localized titles and append aliases, then the reviewed local overlay is applied last. Coverage is strongest for notable games, so it complements rather than replaces Steam data.
