...
The Wikidata worker joins public Wikidata items to the catalog by Steam AppID (P1733), then imports Simplified Chinese labels and aliases. A Steam/PICS localized title always wins; Wikidata can fill a missing title and adds alternate names. One bounded SPARQL query projects only entities that have Chinese labels or aliases, avoiding page scraping and thousands of per-entity requests. The worker identifies itself with a User-Agent, honors rate limits with exponential backoff, and reapplies the reviewed local alias overlay last.
